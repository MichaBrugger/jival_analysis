...
class Calc_Delivery:
    ...
    def calc(self):
        ...
        df_mean_ad.columns = ['TD_Shop', 'TD_Route', 'TD_Driver', 'Avg Delivery']
        df_mean_customers = self.DF_Trip.groupby(['TD_Shop', 'TD_Route', 'TD_Driver'], as_index=False)['Customer_Count'].mean().round(1)
        df_mean_customers.columns = ['TD_Shop', 'TD_Route', 'TD_Driver', 'Avg Customer Count']
        ...
        df_delivery['0 Delivery (%)'] = (df_delivery['Zero_Delivery']/df_delivery['Avg Customer Count']*100).round(1)

        # Naming columns properly
        df_delivery.columns = ['TD_Shop', 'TD_Route', 'TD_Driver', 'Amount of Trips', 'Total Deliveries', 'Avg 0 Delivery',
                               'Avg Delivery','Avg Planned', 'Min Delivery', 'Max Delivery','Avg Customer Count', 'Avg 0 Delivery (%)']
        ...
        df_delivery = df_delivery.drop(['Avg Customer Count', 'Min Delivery', 'Max Delivery', 'Avg Planned',
                                        'Avg 0 Delivery'], axis=1)
